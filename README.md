...
Subscriptions are normal HTTP(S) URLs (and optional local files) declared under **`hosts`** and **`domains`** in the EdgeOS blacklist tree. This project does not ship a curated domain blocklist in-tree; you point sources at community lists and tune with EdgeOS excludes/includes.
...
- **Router install:** use your usual packaging or artifact workflow; **`.payload/post-install.sh`** illustrates CLI commands that provision blacklist sources and a periodic **`update-dnsmasq`** task. **`config.gateway.json`** is a UniFi **`config.gateway.json`** example for USG-style provisioning.
