...
// purgeFiles removes any orphaned adblock files that don't have sources
...
// writeFile saves normalized dnsmasq data to disk.
func (b *bList) writeFile() error {
	...
	if b.size == 0 {
		if err := os.Remove(b.file); err != nil && !os.IsNotExist(err) {
			return err
		}
		return nil
	}
